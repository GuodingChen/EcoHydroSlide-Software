...
switch Vegetation_name
    ...
    case "Normal"
        ...
        Broot = h5read(H5fileName,'/MonthlyData/Monthly_DBroot');
    case "NoVegDynamics"
...
for i_SM = 1 : length(ModelTime)
    ...
    % landslide data need
        ...
        parfor tile_class = 1 : total_tile_number
            Monitor_variable = strcat(Vegetation_name,': tile = ', num2str(tile_class), ' & step = ', TimeMoment);
            ...
            ellipsoid_number = ellipse_density * 16 * A_s / (pi * (min_ae + max_ae) * ...
                (min_be + max_be)); 
...
%         geotiffwrite(Area_Filename, Area_whole_map, Landslide_geoinfo);
        ...
        OutputH5name_moment = strcat(OutputH5name, "_ce=", num2str(c_e), ".h5");
